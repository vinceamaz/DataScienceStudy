...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        # just initialize the layers. there is no connection between the layers here.

        # number of words x number of features as input dim to the model
    ...
    def embedding_lookup(self, t):
        """ Utilize `self.pretrained_embeddings` to map input `t` from input tokens (integers)
            ...
            PyTorch Notes:
                ...
                - The Embedding object module is often used to store word embeddings and retrieve them using indices. The input to the module 
                    is a list of indices, and the output is the corresponding word embeddings. `t` contains the indices of the words in the 
                    batch.

            @param t (Tensor): input tensor of tokens (batch_size, n_features)
                n_features is the embedding vector of each word
        ...
        # t: tensor of shape (batch_size, n_features)
        # embedding:tensor of shape (batch_size, n_features, embedding_size)
        # x: tensor of shape (batch_size, n_features * embed_size)

        embedding = self.pretrained_embeddings(t)
        x = embedding.view(t.size()[0], self.n_features * self.embed_size)
    ...
    def forward(self, t):
        ...
        relu_output = F.relu(embed_to_hidden)
